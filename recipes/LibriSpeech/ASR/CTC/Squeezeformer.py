...
class ConvModule(nn.Module):
    ...
    def __init__(
        ...
        self.sequential = nn.Sequential(
            ...
            Swish(),
            DepthwiseConv1d(in_channels * expansion_factor, in_channels * expansion_factor, kernel_size, stride=1, padding=(kernel_size - 1) // 2),
...
class Squeezeformer(nn.Module):
    ...
    def forward(self, inputs, input_lengths):
        ...
        input_lengths : torch.Tensor 
            (batch)
        """
        input_lengths = (input_lengths * inputs.shape[1]).round().long()
